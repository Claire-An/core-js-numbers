...
function getAverage(value1, value2) {
  return value2 === 0 ? value1 / 2 : value2 * ((value1 / value2 + 1) / 2);
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const denominator2 = Math.sqrt(x1 ** 2 + y2 ** 2);
  if (numerator === 0) return Math.acos(0);
  const result = numerator / (denominator1 * denominator2);
...
function getFibonacciNumber(index) {
  ...
  for (let i = 1; i < index; i += 1) {
    const buff = fibonacciNumber;
    fibonacciNumber += prevFibonacciNumber;
    prevFibonacciNumber = buff;
  }
...
function isPowerOfTwo(num) {
  for (let number = 1; number <= num; number *= 2) {
...
function getCountOfOddNumbers(number) {
  let count = 0;
  let curNumber = 1;
  while (curNumber <= Math.abs(number)) {
    curNumber += 2;
    count += 1;
  }
  return count;
